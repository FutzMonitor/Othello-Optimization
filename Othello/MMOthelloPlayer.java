...
public class MMOthelloPlayer extends OthelloPlayer implements MiniMax {
    ...
    public int minValue(int depth, GameState state) {
        if (depth>=depthLimit) {
            staticEvaluations++;
        ...
        for (Square square:validSquares) {
            if (square!=null) {
                ...
                min = Math.min(min, maxValue(depth+1, gs));
            }
        }
        return min;
    }

    public int maxValue(int depth, GameState state) {
        if (depth>=depthLimit) {
            staticEvaluations++;
            return staticEvaluator(state);
        }
        ...
        for (Square square:validSquares) {
            if (square!=null) {
                generatedNodes++;
                GameState gs = state.applyMove(square);
                max = Math.max(max, minValue(depth+1, gs));
            }
        }
        return max;
    }
